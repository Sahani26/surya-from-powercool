...
/* ================= ENV ================= */
const OPENROUTER_API_KEY_FF = process.env.OPENROUTER_API_KEY_FF;
...
export async function GET(req: Request) {
  try {
    ...
    if (!OPENROUTER_API_KEY_FF) {
      throw new Error("OPENROUTER_API_KEY_FF missing");
    }
...
`;
    ...
    const topicsResponse = await fetch(
      "https://openrouter.ai/api/v1/chat/completions",
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${OPENROUTER_API_KEY_FF}`,
